...
# Core System Engine Integrations (Day 18, 19, 20 & 21)
...
async def system_event_callback(event_envelope: dict):
    """Day 19-21 Core Event Callback handler."""
    print(f"📡 [EVENT INTERCEPTED]: {event_envelope.get('event_type')} from {event_envelope.get('source_node')}")

async def run_persistent_orchestrator(workflow_uuid, routing_matrix, state_memory, event_broker, metrics_exporter):
    """Processes loops, isolates mock crashes, and exports telemetry metrics data streams in real time."""
    ...
    for node in routing_matrix:
        ...
        if node == "Analysis_Agent_Node":
            ...
            # Day 21 Update: Track the isolation action inside the analytics exporter counters
        ...
        # Day 21 Telemetry Hooks: Calculate processing latency and increment metrics registries
...
async def main():
    ...
    # Day 21 Metrics Exporter instance allocation
    ...
    try:
        async with RedisDistributedLock(redis_runtime_client, relational_workflow_uuid, lease_time_sec=60):
            ...
            await run_persistent_orchestrator(relational_workflow_uuid, SYSTEM_ROUTING_MATRIX, state_memory, event_broker, metrics_exporter)
            
            await asyncio.sleep(0.2)
            await event_broker.stop_background_listener()
            
            # Day 21: Compile and print live metrics payload matrix structural dump
